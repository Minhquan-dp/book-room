...
public class LoginProcess {
  ...
  public LoginResponse login(@Validated @RequestBody LoginInfo info, BindingResult bindingResult) {
    ...
    if (bindingResult.hasErrors()) {
      bindingResult.getAllErrors().forEach(error -> {
        response.addError(new errorDto("VALIDATION_FAILED", error.getDefaultMessage()));
      });
      return response;
    }
    // check info
